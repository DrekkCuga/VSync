...
function hostAlert(titleMessage) {
    if (host || freeForAll) {
        $.notify({
        ...
        $.notify({
            title: '<strong>' + titleMessage + ': </strong>',
            icon: 'fas fa-users',
            message: " Error - Must be the host"
        }, {
                type: 'danger',
                delay: 400,
                animate: {
...
function syncVideo(roomnum) {
    if (host || freeForAll) {
        ...
        socket.emit('send message', "[Info] Synced video");
    }
}
...
function changeVideoParse(roomnum) {
    if (host || freeForAll) {
        ...
        socket.emit('send message', "[Info] Video changed");
    }
}
...
function loveLive(roomnum) {
    if (host || freeForAll) {
        ...
        socket.emit('send message', "[Info] Changed video to random K-pop");
    }
}
...
function changePlayer(roomnum, playerId) {
    if (host || freeForAll) {
        if (playerId != currPlayer) {
            ...
            socket.emit('send message', "[Info] Changed player");
        }
    }
}
