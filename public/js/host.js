...
function changeHost(roomnum) {
    if (!host){
        ...
        socket.emit('send message', "[Info] Taken host");
    }
}
...
//enable/disable ffa
socket.on('enableFreeForAll', function () {
    freeForAll = true;
});
socket.on('disableFreeForAll', function () {
    freeForAll = false;
});
//enable/disable free for all
function toggleFreeForAll() {
    if (host) {
        socket.emit('free for all');
        if (freeForAll) {
            socket.emit('send message', "[Info] Disabled free for all");
        } else {
            socket.emit('send message', "[Info] Enabled free for all");
        }
...
function disconnected() {
    ...
    if (notifyfix) {
        disconnectedAlert();
        socket.emit('send message', "[Info] I'm out of sync, please sync me host");
    } else {
        notifyfix = true
    }
}
